...
@section('main_content')
    ...
    <div class="content-header">
	 <h3 class="box-title">
	 	<i class="fa fa-address-card"></i> {{trans('labels.services')}} 
	  </h3>		  
	</div>
    <section class="content">
    	
    			
    			<form enctype="multipart/form-data" method="post" action="{{ route('services.store') }}" style="background: #fff; border-radius: 10px; padding:10px;">

                    <div class="form-group"> 
                        <label for="title">{{ trans('labels.title') }}</label>
                        <input type="text" id="title" name="name" placeholder="{{ trans('labels.title') }}" class="form-control" required="required">
                    </div>
