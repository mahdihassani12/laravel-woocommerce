...
class productController extends Controller
{
     public function index(Request $request)
    {
        ...
        $data['search']=$search;
        return view('backend.product.index')->with('products',Product::fetchProducts($search))->withData($data);
    }
    ...
    public function store(Request $request)
    {
        $this->validate($request,[
            ...
            'long_desc' => 'required',
            'featured_image' => 'required|mimes:jpg,jpeg,gif,png,svg|max:1024',
    ...
    public function deleteProduct($id)
    {
	   try {
        ...
        if(!is_null($file) && file_exists(public_path('uploads/guide/'.$file))){
             unlink(public_path('public/uploads/guide/'.$file));
          }
        ...
        if(!is_null($feature_image) && file_exists(public_path('uploads/products/'.$feature_image))){
         ...
         foreach ($gallery as $gl) {
          if($gl!="" && file_exists(public_path('uploads/gallery/'.$gl))){
             unlink(public_path('public/uploads/gallery/'.$gl));
             }  
          }       
