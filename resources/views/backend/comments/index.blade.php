            	...
            	<table class="table downloads_table">
                  ...
                  @else
                        ...
                        <div class="row_options">  
                           <form style="display:inline;" action="{{ route('comments.update',$comment->id) }}" method="post" class="comment_approve_form">
                             @csrf
                             @method('PUT')
							@if($comment->status != 1) 
                      ...
                      <td>
                         @if($comment->status == 1)
                            ...
                            <span style="background: lightgreen" class="status badge">{{trans('labels.reviewed')}}</span>

                         @else

                            <span style="background: red" class="status badge">{{trans('labels.new')}}</span>
