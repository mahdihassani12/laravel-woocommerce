<?php $user=Auth::user();?>
 @if(Auth::user()->role_id==2)
    <script type="text/javascript">
      window.location.href = '{{asset("/my_account")}}';
    </script>    
@endif
<?php 
$requests = DB::table('tb_product_request')->where('status','1')->limit(8)->get();
?>
      ...
      <div class="navbar-custom-menu">
        <ul class="nav navbar-nav">
                    ...
                    @if($request->status==1)
                      <li>
                        <a href="{{ route('requests.index') }}">
                          <i class="fa fa-cube text-aqua"></i> {{ $request->name }} - {{ $request->product_name }}</a>
                      </li>
                    @endif
                  @endforeach
                </ul> <!-- request lists -->
          ...
          <li class="dropdown user user-menu">
            <a href="#" class="dropdown-toggle" data-toggle="dropdown">
              <img src="{{asset('public/uploads/users')}}/{{$user->photo}}" class="user-image" alt="User Image">
