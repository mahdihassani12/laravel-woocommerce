    ...
    <div class="container">
     	<div class="row">
          ...
          <div class="download_file_section">
            <ul>
               <li> 
            {{ trans('labels.file_size') }} : <span>{{ number_format($download->size / 1048576,2). '  ام  بی' }} 
          </span></li>

          <li>{{trans('labels.download')}} : <a href="{{asset('public/uploads/files')}}/{{$download->file}}" class="downloading_file" download>
         ...
         <div class="related_data">
              ...
              <div class="row">
                @foreach($data['related'] as $rel)
                  <div class="col-md-3">
                      <img src='{{ asset("public/uploads/files_image/$rel->image") }}' style="max-width:100%">
                      <h4>
                          <a href='{{ asset("view-download?id=".$rel->id."&download=".$rel->name) }}' > {{$rel->name}} </a></h4>
                  </div>
                @endforeach  
              </div>
           </div>  
